...
clf = Ridge(alpha=1).fit(X_train_imputed, y_train)
y_pred = clf.predict(X_test_imputed) # tính y dự đoán

print("R2: %.9f" % r2_score(y_test, y_pred)) #mang

# In ra sự chênh lệch giữa giá trị thực tế và dự đoán
print("Thuc te  | Du doan  | Chenh lech")
for i in range(len(y_test)):
    print(f"{y_test[i]:<10.3f} {y_pred[i]:<10.3f} {abs(y_test[i] - y_pred[i]):<10.3f}")
